...
def test_insert_file():
    with TestClient(app) as client:
        # Set a valid image url to test this test cases
        content_with_valid_image_url = b'url\nPut valid image url here'
        _test_file(client, 'valid_image_url.csv', content_with_valid_image_url,
                   {'cnt_url': 1, 'cnt_image': 1})
...
def test_image():
    with TestClient(app) as client:
        ...
        response = client.get(f"/images/{images[0]['id']}")
        assert response.status_code == 200
...
def test_region():
    with TestClient(app) as client:
        ...
        regions[0]['use'] = False
        response = client.put(f"/regions/{regions[0]['id']}", json=regions[0],
